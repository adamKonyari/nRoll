...
public class DatabaseProfileDao extends AbstractDao implements ProfileDao {
    ...
    @Override
    public List<Profile> findAllByNameOrEmail(String searchParam) throws SQLException {
        if (searchParam == null || "".equals(searchParam)) {
            throw new IllegalArgumentException("Email cannot be null or empty");
        }
        List<Profile> profileList = new ArrayList<>();
        String sql = "SELECT * FROM profile WHERE firstname = ? OR middle_name = ? OR last_name = ? OR email = ?";
    ...
    private Profile fetchProfile(ResultSet resultSet) throws SQLException {
        ...
        return new Profile(id, email, firstname, middleName, lastName, dateOfBirth, institutionId, majorId, studentId, socialSecurityNumber, taxNumber);
    }
}
